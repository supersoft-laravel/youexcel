...
<style>
    ...
    .jobs-header::before {
        ...
        width: 100%;
        height: 100%;
        background: linear-gradient(rgba(0, 40, 104, 0.8), rgba(0, 40, 104, 0.9)),
...
<section class="jobs-header">
    <div class="container">
        <div class="jobs-title">
            <h2>Career Connect - Candidate Board (For Employers)</h2>
    ...
    <div class="container">
        <div class="available-jobs-wrapper text-center mb-5">
            <div class="available-jobs-badge d-inline-flex align-items-center justify-content-center">
                ...
                <span>Available Candidate(s):</span>
                <strong class="ms-2">{{ count($candidates) }}</strong>
            </div>
        </div>
        ...
        @php
            function maskName($name) {
                return substr($name, 0, 3) . str_repeat('*', max(strlen($name) - 3, 0));
            }
        
            function maskPhone($phone) {
                return substr($phone, 0, 4) . str_repeat('*', max(strlen($phone) - 4, 0));
            }
        ...
        @endphp
            @forelse($candidates as $candidate)
                ...
                <div class="col-md-4 d-flex">
                    <div class="job-card w-100">
                        <h5>{{ isset($candidate->name) ? maskName($candidate->name) : 'User' }}</h5>
                        <div class="company-name">
                            {{ isset($candidate->phone) ? maskPhone($candidate->phone) : 'N/A' }}
                            -
                            <small>{{ isset($candidate->email) ? maskEmail($candidate->email) : 'N/A' }}</small>
                        </div>
                        <div class="ref-no"><strong>Ref#:</strong> {{ $candidate->ref_no }}</div>

                        <div class="expertise">
                            <strong>Expertise:</strong><br>
                            @php
                                $expertiseArray = is_array(json_decode($candidate->expertise, true))
                                    ? json_decode($candidate->expertise, true)
                                    : [];
                        ...
                        <div class="mt-3">
                            <a href="{{ route('career.connect.form', ['formType' => 'employer_partnership']) }}?candidate_ref={{ $candidate->ref_no }}" class="btn btn-apply">Interested</a>
                        </div>
                    </div>
                </div>
                @endif
            @empty
                <div class="col-12">
                    <div class="no-jobs">
                        <i class="bi bi-briefcase"></i> No candidates available at the moment. Please check back later.
                    </div>
                </div>
            @endforelse
        </div>
